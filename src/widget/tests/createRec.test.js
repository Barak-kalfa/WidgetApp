...
describe("testing createRecommendation()", () => {
  ...
  const TEST_ELEMENT = createRecommendation(testRec, TEST_SETTINGS.type[testRec.origin])
  it("should create the right element type", () => {
    expect(TEST_ELEMENT.nodeName).toBe("DIV");
  });

  it("should give the element the right class", () => {
    expect(TEST_ELEMENT.className).toBe(TEST_SETTINGS.type[testRec.origin].className);
  });
...
describe('testing createThumb()', () => { 
  ...
  TEST_ELEMENT.innerHTML = createThumb(testRec, TEST_SETTINGS.type[testRec.origin]);
  it('should return a valid HTML element with the right class', ()=>{
    const thumbElement = TEST_ELEMENT.querySelector(TEST_SETTINGS.type[testRec.origin].thumbElementType);
